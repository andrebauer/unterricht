...
Bei anderen Linux-Distributionen, die z.{nbsp}B. auf Redhat basieren,
müssen die Befehle, die zur Installation neuer Pakete dienen,
angepasst werden, d.{nbsp}h. es muss für Redhat `yum` statt `apt`
...
<2> Beim ersten Login eines Clienten auf einem SSH-Server wird der
    ECDSA-Key-Fingerprint ausgegeben.
...
per `ssh` anzumelden, ist natürlich auf möglich sich remote über ein
Netzwerk auf einem anderen System anzumelden.

Für das folgenden Beispiel müssen zwei VMs miteinander
vernetzt (siehe Information IT-BS-VM-INFO-3.1) und auf dem
...
Im unterschied <<ssh-mit-linuxgast>> wird dem SSH-Befehl Port als
Option mit `-p 8022` eine Portnummer, die für die Verbindung mit dem
