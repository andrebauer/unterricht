...
. Richten Sie eine Linux-VM mit SSH-Server und einem Host-only Adapter
  ...
  Beispielen und anderen Quellen abweichen.
  Testen Sie wie in <<aufgabe-ssh-gast, Aufgabe 1>> den SSH-Server
  vom Host-System sowie von einer zweiten Linux-VM.
...
. Richten Sie eine Linux-VM mit SSH-Server, NAT-Netzwerk und Port-Weiterleitung
  ...
  Beispielen und anderen Quellen abweichen.
. Kopieren Sie Dateien zwischen einer zwei Linux-VM sowie einer Linux-VM und
  dem Host über die Port-Weiterleitung. Kopieren Sie Dateien sowohl zum als
  auch vom SSH-Server.
****