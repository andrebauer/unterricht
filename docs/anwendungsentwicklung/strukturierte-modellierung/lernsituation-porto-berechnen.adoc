...
Stellen Sie Ihre Lösung als
--
. Struktogramm,
. Programmablaufplan und
. Pseudocode dar.
--
...
** `int portoOfClass (String classification)`
+
Gibt anhand einer Klasse das entsprechende Porto zurück.


. Entwickeln sie mithilfe der Funktion `classify` und `portoOfClass`
folgendes:

** eine Funktion, die anhand der Maße und des Gewichts das Porto
   ausgibt.

** eine Funktion, die anhand der Maße, des Gewichts
   und des Portos prüft, ob ein Brief ausreichend frankiert ist.
