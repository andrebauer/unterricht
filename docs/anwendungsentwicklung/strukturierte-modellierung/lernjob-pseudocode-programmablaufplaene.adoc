...
Bevor die Einheit Watt zu Angabe der Leistung von Maschinen
...
heute noch gebräuchlich ist. Das Programm soll eine Angabe in Pferdestärken
in Kilowatt (kW) umrechnen und ausgeben.

[stem]
++++
1\;\text{PS} = 735{,}498\;\text{W}
++++
...
Als Berechnungsgrundlage dient jeweils der Reifenaußendurchmesser in Zoll
...
s &= d \cdot
2{,}54 \frac{\text{cm}}{\text{Zoll}} \cdot \pi \cdot
\text{Umdrehungen} \cdot \frac{\text{m}}{100\;\text{cm}} \\

v &= d \cdot
2{,}54 \frac{\text{cm}}{\text{Zoll}} \cdot \pi \cdot
\text{Umdrehungen} \cdot \frac{1}{3\;\text{s}} \cdot
20\frac{\text{s}}{\text{min}} \cdot 60\frac{\text{min}}{\text{h}} \cdot
...
Die folgenden Pharmazentralnummern können als Testdaten verwendet wreden. 
