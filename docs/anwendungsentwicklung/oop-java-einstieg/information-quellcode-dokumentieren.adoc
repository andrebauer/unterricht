...
Quelltexte sollten so verfasst werden, dass sie gut lesbar sind.
Dazu sollten die Konventionen, wie in einer Programmiersprache Quellcode
...
Zu disen Konvention zählen u.{nbsp}a. die korrekte Einrückung
...
Die Tags `@author` und `@version` werden in dem Kommentar verwendet, das eine
Klasse oder eine Schnittstelle beschreibt, sie werden daher üblicherweise
zu Beginn einer Java-Datei verwendet.
...
<3> Der Bezeicher nach dem Tag @param, hier ist es `l`, muss
...
folgende Aufruf die HTML-Dokumentation aller Klassen und Schnittstellen
erzeugt:
...
in einem
https://www.video2brain.com/de/tutorial/die-api-dokumentation-und-javadoc[Screencast zur API-Dokumentation mit Javadoc] wird dies anhand von Eclipse
vorgeführt.