...
fort und sendet L2 die Nachricht _einschalten()_.
...
[[objektdiagramm-1]]
.Objektdiagramm für einen Aufbau mit einem Schalter und drei Lampen
[plantuml, objektdiagramm-schalter-drei-lampen, svg]
....
...
* es wäre unflexibel, man möchte mit derselben Software auch Schaltungen
...
dem Schalter und `ausschalten()` bei Lampe notieren, also bei der
Klasse, der man die jeweilige Nachrichten sendet.
Die Daten (Attribute der Objekte bzw. Klassen) werden im Klassendiagramm
...
Aus dem Objektdiagramm in <<objektdiagramm-1>> haben die Objekte das
Attribut `zustand` mit den möglichen Werten `ON` und `OFF.`
Anstatt eines vorhandenen Java-Datentyps wie `int`, `boolean` oder `string`
erstellen wir mithilfe einer Aufzählung (engl. enumeration)
...
englischer Sprache verwendet wird. 
...
Diesem entspricht das folgende Java-Programm:
...
Wirklich? Nein, noch nicht ganz, denn wie kann der Schalter die Lampen
verwenden?
